...
INTEGRATION_OPENCLAW_MODEL_PRIMARY="${INTEGRATION_OPENCLAW_MODEL_PRIMARY:-openai/gpt-5}"
INTEGRATION_OPENCLAW_OPENAI_API_KEY="${INTEGRATION_OPENCLAW_OPENAI_API_KEY:-integration-test-key}"
...
HOME="${HOME_DIR}" "${CLAWFARM_BIN}" run "${INTEGRATION_IMAGE_REF}" \
  ...
  --openclaw-model-primary="${INTEGRATION_OPENCLAW_MODEL_PRIMARY}" \
  --openclaw-openai-api-key="${INTEGRATION_OPENCLAW_OPENAI_API_KEY}" \
  --no-wait \
  >"${RUN_LOG}" 2>&1
RUN_EXIT=$?
set -e
